...
class Planificador(Frame):
    ...
    def initUI(self):
        self.master.title("Tkinter Color")
        ...
        self.scrolly.grid(row=1, column=1, sticky='nsew')  #grid scrollbar in master, but
        self.canvas['yscrollcommand'] = self.scrolly.set   #attach scrollbar to frameTwo
        self.scrollx=Scrollbar(self.master, orient="horizontal",command=self.canvas.xview)
        self.scrollx.grid(row=2, column=0, sticky='nsew')  #grid scrollbar in master, but
        self.canvas['xscrollcommand'] = self.scrollx.set   #attach scrollbar to frameTwo

        self.canvas.create_window((0,0),window=self.listFrame,anchor='nw')
        self.listFrame.bind("<Configure>", self.AuxscrollFunction)
        self.scrolly.grid_forget()                         #Forget scrollbar because the number of pieces remains undefined by the user. But this not destroy it. It will be "remembered" later.
        self.scrollx.grid_forget()                         #Forget scrollbar because the number of pieces remains undefined by the user. But this not destroy it. It will be "remembered" later.

        self.canvas.pack(side="left")
        self.scrolly.grid(row=1, column=1, sticky='nsew')
        self.scrollx.grid(row=2, column=0, sticky='nsew')
        self.aceptar_piezas_ok()

        self.title = Label(self.frameOne, text = " Amount of pieces ", justify="center")
    ...
    def aceptar_piezas_ok(self):
        ...
        maxRow=36
        for color in COLORS:
          e = Entry(self.listFrame, background=color, font=(None, -FONT_SIZE), width=15 )
          e.insert(0, color)
